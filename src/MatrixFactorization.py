...
# data5=pd.DataFrame()
# for i in range(10):
#     data5=pd.concat([data5,data])





def get_prediction(data,K,steps,learning_rate,r_lambda,id,channelid) :
    #data['gudok'] = 1
    ...
    P, Q=matrix_factorization(df_user_channel.values,K=K, steps=steps, learning_rate=learning_rate, r_lambda=r_lambda)
    ...
    def recomm_movie_by_userid(pred_df,userId,unseen_list,top_n=10):
        ...
        recomm_movies=pred_df.loc[userId,unseen_list]
        #.sort_values(ascending=False)[:top_n]
        return recomm_movies

    #######################################
    #사용자가 구독하지 않은 채널명 추출
    unseen_list=get_unseen_movies(df_user_channel,1)

    #잠재 요인 협업 필터링으로 채널 추천
    recomm_movies=recomm_movie_by_userid(title_pred_matrix,1,unseen_list,top_n=10)
...
# #
# recomm_df.to_csv("C:/Users/윤유진/OneDrive - 데이터마케팅코리아 (Datamarketingkorea)/바탕 화면/추천시스템/output.csv",mode='w', encoding='utf-8-sig')
...
pd.merge(true_df.reset_index(),recomm_df.reset_index(),how='inner',left_on='user_id',right_on='userid').to_csv("C:/Users/윤유진/OneDrive - 데이터마케팅코리아 (Datamarketingkorea)/바탕 화면/추천시스템/compare_중복x_sns.csv",mode='w', encoding='utf-8-sig')

sum

# get_prediction(data,K=100,steps=200,learning_rate=0.01,r_lambda=0.01,id=1,channelid='"188236564542293')
