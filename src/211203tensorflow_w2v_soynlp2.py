...
##################################################################
with open("C:/Users/윤유진/OneDrive - 데이터마케팅코리아 (Datamarketingkorea)/문서/GitHub/NLPProjects/recommend/buzz_data_202111.pkl","rb") as fr:
    data = pickle.load(fr)
...
nouns = noun_extractor.train_extract(datalist_clean)

# clean_m_text=[noun_extractor.train_extract(x) for x in tqdm(datalist_clean)]
...
list_tokenized=[]
for i in range(len(datalist_clean)):
    sent = datalist_clean[i]
    # print(tokenizer.tokenize(sent))
    tokenized = [x for x in tokenizer.tokenize(sent) if x in nouns.keys()]
    list_tokenized.append(tokenized)


preprocessed_text_count=list(itertools.chain(*list_tokenized))
...
for n, c in count.most_common(33000):
    dics = {'tag': n, 'count': c}
    if len(dics['tag']) >= 2 and dics['count']>=5 :
...
preprocessed_text=[' '.join([j for j in list_tokenized[i] if j in tags]) for i in tqdm(range(len(list_tokenized)))]
...
preprocessed_text[0]
vocab_reverse = {i:x for x,i in tqdm(vocab.items())}
# vocab_reverse[364]

vocab['pad'] = 0
vocab_size = len(vocab)

file_path_vocab="../vocab_soynlp.json"
file_path_reverse="../vocab_reverse1_soynlp.json"
